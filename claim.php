#Script Recoder By LeeProject
#Nothing Special For Me WTF!!!
...
function call($addr) {
 ...
 $data= "faucetclaim=$addr";
    ...
    if(!preg_match("/Error/", $body)){
       ...
       $res['status'] = false;
        $res['message'] = "Ip di Banned.. Kamu kurang Tamvan!"; 
    }else{
        $res['status'] = false;
        $res['message'] = $claim;
    }
...
for($i=0; $i<100; $i++) {
    echo "[" . date("H:i:s") . "]  Sebentar, Akan Aku Coba Claimkan Buat Kamu..\n";
    $claim = call($btc);
    //print_r($claim);
    ...
    if (isset($claim['info'])) {
        ...
        if ($claim['status'] == 1) {
            echo $GR . "[" . date("H:i:s") . "] WOW Kamu Sungguh Mempesona ! " . $claim ['message'] . $CC . "\n";
        } elseif ($response['status'] == 0) {
            echo $RD . "[" . date("H:i:s") . "] Kurang Tamvan ! " . $claim['message'] . $CC . "\n";
        }
    } else {
        exit("[" . date("H:i:s") . "] " . $RD . "Invalid cookies!" . $CC);
    }
    echo "[" . date("H:i:s") . "] Tunggu Semenit, Aku Sedang Melihat Ketamvananmu...\n";
    sleep(60);
}
